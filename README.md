...
## 요구사항 정리
1. 평일 및 주말(토, 일, 공휴일) 비상근무가 있음.

### 입력
1. 평일 비상근무 리스트 및 주말 비상근무 리스트가 있음.
 - 각 비상근무 리스트에 인원은 중복될 수 없음.
 - 순번대로 돌아감
 - 만약 연속으로 2일 근무하게 될 경우, 해당일의 종류(평일/주말)의 다음 담당자와 순서를 변경
 - 이름에 중복값 불가
 - 이름은 최대 5자까지
 - 인원은 최소 5명 이상, 최대 35명 이하.

### 출력
1. 특정 월(숫자) 및 시작 요일(1일의 요일)에 따라 해당 요일의 리스트를 출력
2. 잘못된 입력인 경우, [ERROR]를 포함한 에러문구 출력
